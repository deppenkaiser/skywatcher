...
static uint32_t _timer_frequency = 0;
static uint32_t _cpr[3] = {0};
static double _siderial_w_deg_per_s = 0.0;
static double _siderial_w_error_1_level_deg_per_s = 0.0;
static double _siderial_w_error_2_level_deg_per_s = 0.0;
static double _max_exposure_time_s = 0.0;
static pthread_t _thread_handle_axis_1 = THREADING_INVALID_THREADHANDLE;
...
PRIVATE_FUNC(bool, set_motion_mode(enum skywatcher_axis axis, uint8_t mode_bits));
PRIVATE_FUNC(void, trigger_goto_callback(enum skywatcher_axis axis, enum skywatcher_mode last_mode, struct skywatcher_axis_status* last_status));
PRIVATE_FUNC(void, update_position(enum skywatcher_axis axis, struct skywatcher_axis_status* axis_status));
PRIVATE_FUNC(void*, mount_thread(void* data));
PRIVATE_FUNC(void, get_buffer_out(data_t value, data_t buffer_out));
PRIVATE_FUNC(int32_t, convert_data(data_t value));
PRIVATE_FUNC(void, put_buffer_to_buffer(data_t out, data_t in));
PRIVATE_FUNC(void, speed_to_preset(data_t preset_out, enum skywatcher_axis axis, double angular_speed_degrees_per_s));
PRIVATE_FUNC(bool, set_speed(enum skywatcher_axis axis, double angular_speed_degrees_per_s));
PRIVATE_FUNC(void, calculate_siderial_angular_speed_deg_per_s());
PRIVATE_FUNC(double, calculate_max_exposure_time_s(double pixel_size_um, double focal_length_mm));
PRIVATE_FUNC(bool, initialize_axis(enum skywatcher_axis axis));
PRIVATE_FUNC(bool, get_motor_board_version(enum skywatcher_axis axis));
PRIVATE_FUNC(bool, get_timer_frequency(uint32_t* frequency));
PRIVATE_FUNC(bool, get_cpr(enum skywatcher_axis axis, uint32_t* cpr));
PRIVATE_FUNC(bool, get_axis_status(enum skywatcher_axis axis, data_t buffer_out));
PRIVATE_FUNC(int32_t, convert_position_data(data_t value));
...
void skywatcher_start_thread(skywatcher_status_t status, void* user_data)
{
    ...
    _thread_handle_axis_1 = threading_thread_create(_mount_thread, (void*)(intptr_t) SA_AXIS_1);
    _thread_handle_axis_2 = threading_thread_create(_mount_thread, (void*)(intptr_t) SA_AXIS_2);
}
...
bool skywatcher_initialize_axis()
{
    ...
    if (_initialize_axis(SA_AXIS_1) && _initialize_axis(SA_AXIS_2))
    {
        if (_get_timer_frequency(&_timer_frequency) && _get_cpr(SA_AXIS_1, &_cpr[SA_AXIS_1]) &&
                _get_cpr(SA_AXIS_2, &_cpr[SA_AXIS_2]))
        {
            _calculate_siderial_angular_speed_deg_per_s();
            is_ok = true;
        }
    }
...
bool skywatcher_set_ra_siderial_speed()
{
    ...
    if (_set_speed(SA_AXIS_1, _siderial_w_deg_per_s + _siderial_w_error_1_level_deg_per_s))
    {
        if (_set_motion_mode(SA_AXIS_1, MOTION_TRACKING))
        {
...
bool skywatcher_set_axis_speed_and_start(double w_deg_per_s, enum skywatcher_axis axis)
{
    ...
    if (_set_speed(axis, fabs(w_deg_per_s)))
    {
        ...
        {
            _set_motion_mode(axis, MOTION_TRACKING | (direction ? MOTION_CCW : 0));
...
bool skywatcher_set_axis_sleep(enum skywatcher_axis axis, bool sleep)
{
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_set_axis_sleep");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_set_aux(enum skywatcher_axis axis, bool on)
{
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_set_aux");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_start_motion(enum skywatcher_axis axis)
{
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_start_motion");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_stop_motion(enum skywatcher_axis axis)
{
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_stop_motion");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_instant_stop(enum skywatcher_axis axis)
{
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_instant_stop");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_get_axis_status(enum skywatcher_axis axis, skywatcher_axis_status_t status)
{
    data_t buffer_out = {0};
    threading_critical_section_lock(&_cs);
    bool is_ok = _get_axis_status(axis, buffer_out);
...
bool skywatcher_get_speed(enum skywatcher_axis axis, double* angular_speed_degrees_per_s)
{
    ...
    _get_buffer_out(value, buffer_out);
    char* endptr = NULL;
    ...
    *angular_speed_degrees_per_s = counts_per_s * 360.0 / (double) _cpr[axis];
    bool is_ok = _check_error(buffer_out, "skywatcher_get_speed");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_get_position(enum skywatcher_axis axis, int32_t* position)
{
    ...
    _get_buffer_out(value, buffer_out);
    *position = _convert_position_data(value);
    bool is_ok = _check_error(buffer_out, "skywatcher_get_position");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_set_position(enum skywatcher_axis axis, int32_t position)
{
    ...
    _put_buffer_to_buffer(buffer, value);
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(buffer, SA_NONE, buffer_out);
    bool is_ok = _check_error(buffer_out, "skywatcher_set_position");
    if (is_ok)
    {
        char string[256] = {0};
...
bool skywatcher_get_axis_position(enum skywatcher_axis axis, int32_t* position)
{
    ...
    _get_buffer_out(value, buffer_out);
    *position = _convert_position_data(value);
    bool is_ok = _check_error(buffer_out, "skywatcher_get_axis_position");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_set_goto_target(enum skywatcher_axis axis, int32_t target)
{
    ...
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(buffer, SA_NONE, buffer_out);
    bool is_ok = _check_error(buffer_out, "skywatcher_set_goto_target");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
bool skywatcher_get_goto_target(enum skywatcher_axis axis, int32_t* target)
{
    ...
    _get_buffer_out(value, buffer_out);
    *target = _convert_position_data(value);
    bool is_ok = _check_error(buffer_out, "skywatcher_get_goto_target");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}
...
static const char* _error_code_to_string(char code)
{
    switch (code)
    {
...
static bool _check_error(data_t buffer_out, const char* function_name)
{
    if (buffer_out[0] != '!')
    {
        return true;
    ...
    LOG(MODULE_ID, "skywatcher error: %s, %s", function_name, _error_code_to_string(buffer_out[1]));
    return false;
}

static bool _set_motion_mode(enum skywatcher_axis axis, uint8_t mode_bits)
{
    data_t buffer_in = {0}, buffer_out = {0};
    ...
    bool is_ok = _check_error(buffer_out, "skywatcher_set_motion_mode");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static void _trigger_goto_callback(enum skywatcher_axis axis, enum skywatcher_mode last_mode, struct skywatcher_axis_status* last_status)
{
...
static void _update_position(enum skywatcher_axis axis, struct skywatcher_axis_status* axis_status)
{
    int32_t position = 0;
...
static void* _mount_thread(void* data)
{
    ...
    {
        _update_position(axis, axis_status);
        _trigger_goto_callback(axis, last_status.mode, &last_status);
...
static void _get_buffer_out(data_t value, data_t buffer_out)
{
...
static int32_t _convert_data(data_t value)
{
    char* endptr = NULL;
    return strtol(value, &endptr, 16);
}

static void _put_buffer_to_buffer(data_t out, data_t in)
{
    out[3] = toupper(in[4]);
    out[4] = toupper(in[5]);
    out[5] = toupper(in[2]);
...
static void _speed_to_preset(data_t preset_out, enum skywatcher_axis axis, double angular_speed_degrees_per_s)
{
...
static bool _set_speed(enum skywatcher_axis axis, double angular_speed_degrees_per_s)
{
    data_t buffer_in = {0}, buffer_out = {0};
    bool is_ok = false;
    threading_critical_section_lock(&_cs);
    if (angular_speed_degrees_per_s >= 1.0e-6)
    {
        data_t preset = {0};
        memset(preset, '0', 6);
        _speed_to_preset(preset, axis, angular_speed_degrees_per_s);

        sprintf(buffer_in, CMD_SET_SPEED, axis);
        _put_buffer_to_buffer(buffer_in, preset);
        ...
        is_ok = _check_error(buffer_out, "skywatcher_set_speed");
    }
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static void _calculate_siderial_angular_speed_deg_per_s()
{
    double _siderial_w_real_degree_per_s = 0.0;
    _siderial_w_deg_per_s = 360.0 / SIDERIAL_DAY_S;
    _set_speed(SA_AXIS_1, _siderial_w_deg_per_s);
    ...
    _set_speed(SA_AXIS_1, _siderial_w_deg_per_s + _siderial_w_error_1_level_deg_per_s);
    skywatcher_get_speed(SA_AXIS_1, &_siderial_w_real_degree_per_s);
    _siderial_w_error_2_level_deg_per_s = _siderial_w_deg_per_s - _siderial_w_real_degree_per_s;
}

static double _calculate_max_exposure_time_s(double pixel_size_um, double focal_length_mm)
{
...
static bool _initialize_axis(enum skywatcher_axis axis)
{
    data_t buffer_out = {0};
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(CMD_INIT, axis, buffer_out);
    bool is_ok = _check_error(buffer_out, "_skywatcher_initialize_axis");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static bool _get_motor_board_version(enum skywatcher_axis axis)
{
    data_t buffer_out = {0};
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(CMD_GET_BOARD_VERSION, axis, buffer_out);
    bool is_ok = _check_error(buffer_out, "_skywatcher_get_motor_board_version");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static bool _get_timer_frequency(uint32_t* frequency)
{
    data_t buffer_out = {0};
    data_t value = {0};
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(CMD_GET_TIMER_FREQUENCY, SA_NONE, buffer_out);
    bool is_ok = _check_error(buffer_out, "_skywatcher_get_timer_frequency");
    if (is_ok)
    {
        _get_buffer_out(value, buffer_out);
        *frequency = _convert_data(value);
    }
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static bool _get_cpr(enum skywatcher_axis axis, uint32_t* cpr)
{
    data_t buffer_out = {0};
    data_t value = {0};
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(CMD_GET_CPR, axis, buffer_out);
    bool is_ok = _check_error(buffer_out, "_skywatcher_get_cpr");
    if (is_ok)
    {
        _get_buffer_out(value, buffer_out);
        *cpr = _convert_data(value);
    }
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static bool _get_axis_status(enum skywatcher_axis axis, data_t buffer_out)
{
    threading_critical_section_lock(&_cs);
    _skywatcher_execute(CMD_STATUS, axis, buffer_out);
    bool is_ok = _check_error(buffer_out, "_skywatcher_get_axis_status");
    threading_critical_section_unlock(&_cs);
    return is_ok;
}

static int32_t _convert_position_data(data_t value)
{
    return _convert_data(value) - POSITION_OFFSET;
}
